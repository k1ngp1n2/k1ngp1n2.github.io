// Адрес сервера
const SERVER_URL = "http://89.108.65.123:8080"; // id пользователя

let userID = ""; // Авторизуем пользователя
...
let citiesLoaded = false; // Номер последнего отображенного отзыва
...
NestedMenu.prototype.render = function () {
  ...
  nestedMenuItems.innerHTML = `<span onclick="${this.items[0].href}">${this.items[0].itemName}</span><ul></ul>`; // Обходим все свойства объекта класса Вложенное меню
...
function loadCities(select) {
  let option; // Загружаем список городов
...
function showFlickrImages() {
  // Фото
  const url = "https://api.flickr.com/services/feeds/photos_public.gne?" + "tags=cars&format=json&jsoncallback=?"; // создаем новый элемент jQuery для помещения в него изображения

  let img; // номер индекса первого отображаемого изображения в массиве

  let number = 0; // массив для хранения адресов изображений
...
function httpGet(url) {
  return new Promise(function (resolve, reject) {
    ...
    let xhr = new XMLHttpRequest(); // Создаем обработчик события успешного завершения запроса
    ...
    xhr.onload = function () {
      ...
      if (this.status == 200) {
        ...
        let error = new Error(this.statusText); // Сохраняем код ошибки
...
function loadMenu() {
  ...
  * create_lvlN - команда создания пункта меню уровня N, содержащего вложенное меню */
  httpGet("menu.json").then(result => {
    ...
    const allItems = JSON.parse(result); // Используем полученные данные для создания меню

    let temp; // Текущий уровень элементов меню (0 - главное меню)

    let lvl = 0; // Массивы для хранения пунктов меню для соответствующих уровней глубины вложенности меню
    ...
    let menu = new Menu("menu-id", "menu-class", lvls[0]); // Визуализируем меню на странице сайта
...
function checkForm(form) {
  ...
  let elems = form.elements; // Удаляем сообщение об ошибке заполнения поля с именем пользователя
...
function addAutomobiles(automobilesData) {
  $('#autoShooserControl').change(event => {
    ...
    let val = $(event.target).val(); // Отображаем описание автомобиля
...
function shooseItem(automobilesData) {
  ...
  let autoShooserControl = document.querySelector('#autoShooserControl'); // Добавляем обработчик событий отправки при нажатии на товар в списке каталога
...
function userAuthorization() {
  $.ajax({
    ...
    error: error => {
      ...
      $('<div>').text(error.status + ' ' + error.responseJSON.message).dialog({
        title: 'Ошибка связи'
      });
    }
  });
...
function showBasket(basketData) {
  if (basketData.cart.length == 0) {
    ...
    let basket = `<p>Имя покупателя: ${basketData.user_id}</p><p>Состав корзины:</p>`; // Общая стоимость товаров в корзине

    let value = 0;

    for (let i = 0; i < basketData.cart.length; i++) {
      // Визуализируем каждый товар в корзине
      basket += `<div class="card automobile"><div class="card-body"><p class="card-text">${i + 1}) Наименование: ${basketData.cart[i].product}</p><p class="card-text">Цена: ${basketData.cart[i].price}</p><a href="#" class="btn" id="item${i}">Удалить товар из корзины</a></div></div>`; // Определяем общую стоимость товаров в корзине

      value += Number(basketData.cart[i].price);
    } // Визуализируем общую стоимость товаров в корзине


    basket += `<p>Общая стоимость товаров в корзине: ${value} рублей.</p>`; // Визуализируем содержимое корзины на странице
...
function onClickAddItemToBasket(item, price) {
  $.ajax({
    ...
    error: error => {
      console.error(`${error.status} ${error.responseJSON.message}`);
      $("<div>").text(error.status + " " + error.responseJSON.messag).dialog({
        title: "Ошибка связи"
      });
    }
  });
